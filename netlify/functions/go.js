...
function buildEventPage(ev, related) {
  ...
  const calDetails = encodeURIComponent(desc.substring(0, 200) + (eventLink ? `\n\nJoin: ${eventLink}` : ''));
  const calLocation = encodeURIComponent('Online Event');
  ...
  const industryGradients = {
    'Finance & Banking': 'linear-gradient(135deg, #1C2333, #10B981)',
    'Healthcare & Medical': 'linear-gradient(135deg, #0B1426, #EF4444)',
    'Technology & IT': 'linear-gradient(135deg, #0B1426, #0EA5E9)',
    'Engineering': 'linear-gradient(135deg, #1C2333, #F59E0B)',
    ...
    ? `<p style="margin:32px 0 12px;font-size:18px;font-weight:600;color:#0B1426;">More events you might like</p>
       <table width="100%" cellpadding="0" cellspacing="0">
         ${related.map((r, i) => {
           ...
           return `<tr><td style="padding:6px 0;font-size:14px;color:#0B1426;line-height:1.5;">${i + 1}. <a href="${SITE_URL}/.netlify/functions/go?id=${r.id}" style="color:#1E3A5F;text-decoration:none;font-weight:600;">${r.title}</a> — <span style="color:#64748b;">${rDate} (Online)</span></td></tr>`;
         }).join('')}
       </table>`
    : '';
...
<head>
  ...
  <title>${title} — Conferix Impact</title>
    ...
    <tr><td align="center">
      <table width="640" cellpadding="0" cellspacing="0" style="max-width:640px;width:100%;">
        ...
        <tr><td style="background:linear-gradient(135deg,#0B1426,#1C2333);padding:20px 24px;border-radius:12px 12px 0 0;">
          <table width="100%" cellpadding="0" cellspacing="0">
            <tr>
              <td><a href="${SITE_URL}" style="font-size:20px;font-weight:700;color:#ffffff;text-decoration:none;">Conferix <span style="color:#D4A853;">Impact</span></a></td>
        ...
        <tr><td style="background:#ffffff;padding:32px 24px;">
          ...
          <table cellpadding="0" cellspacing="0" width="100%">
            <tr><td align="center" style="padding-bottom:12px;">
              ${eventLink ? `<a href="${eventLink}" target="_blank" rel="noopener" style="display:inline-block;padding:14px 36px;background:#1E3A5F;color:#ffffff;border-radius:8px;font-size:16px;font-weight:600;text-decoration:none;">Join Event</a>` : ''}
            </td></tr>
            <tr><td align="center">
        ...
        <tr><td style="background:#f8fafc;padding:20px 24px;border-radius:0 0 12px 12px;text-align:center;border-top:1px solid #e2e8f0;">
          <p style="margin:0;font-size:13px;color:#94a3b8;"><a href="${SITE_URL}" style="color:#64748b;text-decoration:underline;">conferix.com/sudan</a> — Free online events for Sudanese professionals</p>
        </td></tr>
...
exports.handler = async (event) => {
  ...
  try {
    ...
    const relatedRecords = await base('Events').select({
      filterByFormula: `AND({start_date} >= "${today}", RECORD_ID() != "${id}", {industry} = "${ev.industry}")`,
